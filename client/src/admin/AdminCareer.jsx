...
function AdminCareer() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) =>
      `${item.title || ""} ${item.organization || ""} ${item.eventType || ""}`
        .toLowerCase()
        .includes(value)
    );
  }, [items, search]);
    ...
    <div className="admin-ui-page">
      ...
      <div className="admin-ui-stats">
        <div className="admin-ui-stat">
          ...
          <small>All milestones</small>
        </div>

        <div className="admin-ui-stat">
          <span>ROLES</span>
          <strong>{items.filter((item) => item.eventType === "ROLE").length}</strong>
          <small>Role events</small>
        </div>

        <div className="admin-ui-stat">
          <span>PROJECTS</span>
          <strong>{items.filter((item) => item.eventType === "PROJECT").length}</strong>
          <small>Project events</small>
        </div>
      ...
      <section className="admin-ui-panel">
        ...
        {filteredItems.length === 0 ? (
          <AdminEmptyState
            icon={<FiActivity />}
            title={search ? "No milestones found" : "No career milestones"}
            description={
              search
                ? "Try another search term."
                : "Add milestones to build your career timeline."
            }
          />
        ) : (
                ...
                <div className="admin-ui-list-content">
                  ...
                  <div className="admin-ui-meta">
                    <span>
                      ...
                      {item.endDate ? ` — ${dateInput(item.endDate)}` : ""}
                    </span>
        ...
        footer={
          <>
            ...
            <button
              ...
              disabled={saving}
            >
              {saving ? <AdminSaving /> : <><FiSave />Save Milestone</>}
            </button>
          </>
        }
      >
        <form id="career-form" onSubmit={save}>
          <div className="admin-ui-form-grid">
            ...
            <label className="admin-ui-field admin-ui-field-full">
              ...
              <input name="organization" value={form.organization} onChange={change} />
            </label>
            ...
            <label className="admin-ui-field">
              <span>Icon key</span>
              <input name="icon" value={form.icon} onChange={change} />
            </label>
